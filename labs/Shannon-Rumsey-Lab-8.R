...
### Action ###
# not stationary! look at the width between spikes and the length of spikes
...
### Action ###
# heights of the vertical lines show the correlation coefficient
# the AFC plot captures the correlation of residuals against themselves over time
...
# the order of autoregression is 3
...
### Action ###
#Our AR estimates are lower than the estimates without this error
...
fig_forecast <- site_data %>%
  ...
  bind_cols(pred = c(fit_full$fitted, preds$mean),
            status = c(rep('obs', nrow(x_train)),
                       rep('pred', nrow(x_test)))) %>%
  filter(date >= ymd('2018-07-10')) %>% # adjust if needed
  ggplot(aes(x = date)) +
  geom_path(aes(y = temp, linetype = status)) +
  geom_path(aes(y = pred), color = 'blue', alpha = 0.5)
...
# The forecasts are good until April. Predictions tend to be much more off for longer term forecasts.
